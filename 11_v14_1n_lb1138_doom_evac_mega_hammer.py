...
def _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log):
    ...
    # 1. Find best evac destination: highest production friendly within reach.
    friendly_candidates = []
    for dst in world.my_planets:
        ...
        friendly_candidates.append((-int(dst.production), int(turns), dst, angle))
    if friendly_candidates:
        friendly_candidates.sort()
        _prod, turns, dst, angle = friendly_candidates[0]
        _commit_fleet(world, moves, spent, target_locked,
