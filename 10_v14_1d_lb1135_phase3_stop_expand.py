...
MEGA_HAMMER_SHIPS_MIN = 300           # source must have >= this many ships to fire
MEGA_HAMMER_TARGET_GARRISON_MAX = 80  # target garrison must be <= this (overkill safe)
MEGA_HAMMER_MAX_TRAVEL = 40           # don't fire if target is more than this many turns away
...
MEGA_HAMMER_TARGET_GARRISON_MAX_ITER_H = 100  # was 80
...
STOP_EXPAND_TURN_MIN_2P = 30        # don't fire before this turn (opening freedom)
...
def _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log):
    ...
    # Find best evac destination: highest production friendly within reach.
    candidates = []
    for dst in world.my_planets:
        ...
        if turns > DOOM_EVAC_MAX_TRAVEL:
            continue
        # rank by (production desc, turns asc)
        candidates.append((-int(dst.production), int(turns), dst, angle))
    if not candidates:
        return False
    candidates.sort()
    _prod, turns, dst, angle = candidates[0]
    _commit_fleet(world, moves, spent, target_locked,
...
DOOM_EVAC_MIN_SHIPS = 5           # don't bother for tiny garrisons
DOOM_EVAC_MAX_TRAVEL = 40         # destination must be reachable in <= this many turns
...
def _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log):
    ...
    for act in actions[:SEARCH_MAX_ACTIONS_TO_PICK * 2]:
        ...
        if world.stop_expanding_2p and tgt is not None and tgt.owner == -1:
            continue
...
def handle_mega_hammer(world, available, spent, target_locked, moves, mode_log):
    ...
    for src in sources:
        ...
        status = mode_log.get(src.id)
        if status and status not in ("cheap-pickup",):
            continue
        ...
        for tgt in world.enemy_planets:
            ...
            score = (int(tgt.production), -int(turns))
