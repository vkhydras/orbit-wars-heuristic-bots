...
SEARCH_MAX_PER_SOURCE = 5       # candidates per source planet to evaluate
SEARCH_MAX_ACTIONS_TO_PICK = 8    # top step actions to commit per turn (peak in 4P)
...
FWD_SCORE_AGG_TURNS = (2, 4, 8, 14, 20)  # V13.2k: added t=2 for early-snipe sensitivity
...
# V13.2a — Reinforce-front terminal stage (Khramtsov #2 / itswin pattern).
# After all offensive/defensive modes complete, any rear (non-frontier)
# friendly planet with surplus ships forwards a fraction to the nearest
# frontier-friendly planet. Cures rear-stockpile paralysis: planets sitting
# behind a frontier accumulate ships that never participate in pressure.
# Frontier = friendly planet that is the closest friend to its closest enemy.
REINFORCE_FRONT_ENABLED = os.environ.get("V132_REINFORCE_FRONT", "1") != "0"
REINFORCE_FRONT_MIN_SHIPS = 12        # below this, forwarding wastes a slot
REINFORCE_FRONT_FRAC = 0.5            # 4P fraction (kept moderate)
REINFORCE_FRONT_FRAC_2P = 0.6         # 2P: push harder forward since one-direction war
REINFORCE_FRONT_MAX_TRAVEL = 30       # don't fire infeasible cross-map flushes
REINFORCE_FRONT_MIN_STEP = 30         # let opening grab neutrals first; flush only mid-game onward
...
def forward_project(world, our_capture_target=None, our_capture_turn=None,
    ...
    # V13.2f: also accept a list of additional pre-committed our actions —
    # enables joint pair/triple evaluation (commit A, score B given A).
    if extra_our_actions:
        for ea in extra_our_actions:
            tid = ea.get("target_id")
            arr = ea.get("arrival_turn")
            shp = ea.get("ships")
            if tid is None or arr is None or shp is None:
                continue
            by_pid[int(tid)].append((int(arr), int(world.player), int(shp)))
    ...
    for t in range(1, horizon + 1):
        ...
        if project_opponent_moves and t % 4 == 0:
            for pid, st in state.items():
                ...
                src_owner = st[0]
                # V13.2g: production-weighted target selection — opponents
                # in reality target high-value, weakly-defended planets, not
                # just whatever is closest. effective_d = d - 1.2*prod -
                # weak-defender bonus. Keeps closest-bias as primary.
                best_eff = float("inf")
                best_d = float("inf")  # actual distance to chosen target (for ETA)
                best_op = None
                for opid, ost in state.items():
                    ...
                    op_prod = prod_by_pid.get(opid, 0)
                    op_garrison = max(0, ost[1])
                    weak_bonus = max(0.0, 8.0 - op_garrison) * 0.4
                    eff_d = d - 1.2 * op_prod - weak_bonus
                    if eff_d < best_eff:
                        best_eff, best_d, best_op = eff_d, d, opid
                if best_op is None:
                    continue
...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
                       use_depth2=False, joint_refine=False, joint_refine_top=4):
    ...
    V13.2f joint_refine: after independent scoring, perform synergy-aware
    re-scoring of the top `joint_refine_top` candidates. We greedily commit
    the #1 candidate then re-score #2..#top by marginal gain GIVEN #1 is
    committed; repeat to depth 2-3. Catches over-redundancy (two actions
    targeting same area, double-defended weak spot) and synergy (two
    captures that together apply enough pressure to flip late-game score).
    """
    ...
    if use_depth2:
        ...
        scored.sort(key=lambda a: (-a["score"], a.get("raw_dist", 0.0)))
    if joint_refine and len(scored) >= 2:
        # V13.2f: synergy-aware refinement. Commit picks one-by-one and
        # re-score the next-best by *marginal* gain given prior picks.
        committed = []
        committed_src = set()
        committed_tgt = set()
        for slot in range(min(joint_refine_top, len(scored))):
            # Score remaining candidates conditional on `committed`
            if committed:
                # Score with all committed actions; use first committed as
                # primary (compat with melis_evaluate single-action arg),
                # rest as extra_our_actions.
                primary = committed[0]
                extras = committed[1:] if len(committed) > 1 else None
                base_committed = melis_evaluate(
                    world, our_step_action=primary,
                    extra_our_actions=extras,
                )
                rescored = []
                for act in scored[slot:]:
                    if act["source_id"] in committed_src or act["target_id"] in committed_tgt:
                        # Conflict — keep independent score so it sorts low
                        rescored.append((act["score"], act))
                        continue
                    # Add act to committed list for evaluation
                    eval_extras = (extras or []) + [act]
                    joint_s = melis_evaluate(
                        world, our_step_action=primary,
                        extra_our_actions=eval_extras,
                    )
                    marginal = joint_s - base_committed
                    if apply_decay and marginal > 0:
                        marginal *= 0.97 ** int(act["arrival_turn"])
                    act["score"] = marginal
                    rescored.append((marginal, act))
                rescored.sort(key=lambda x: (-x[0], x[1].get("raw_dist", 0.0)))
                scored[slot:] = [r[1] for r in rescored]
            # Promote slot's winner to committed
            pick = scored[slot]
            committed.append(pick)
            committed_src.add(pick["source_id"])
            committed_tgt.add(pick["target_id"])
    return scored
...
def melis_evaluate(world, our_step_action=None, horizon=12, future_horizon=8,
                   opp_emit=0.20, extra_our_actions=None):
    ...
    if FWD_SCORE_AGG_ENABLED:
        ...
        final, snaps = forward_project(
            ...
            extra_our_actions=extra_our_actions,
        )
        total = 0.0
        count = 0
        for t in snap_turns:
...
def _neutral_blocked_by_cap(world, target):
    """V12.9 cap55: in 2P, ignore any neutral with CURRENT ships >= 55. Only
    consider once an enemy attack drops it below the cap. No wounded
    exception — strict by current count. 4P keeps the legacy wounded-watchlist
    logic at NEUTRAL_HARD_CAP_4P."""
...
def _endgame_roi_ok(world, target, ships, turns):
    ...
    return expected_growth > float(ships)
...
def plan_solo_capture(world, src, tgt, max_avail, max_travel):
    ...
    ships = max(min_floor, need + EXPAND_MIN_MARGIN)
...
def handle_comet_evac(world, available, spent, target_locked, moves, mode_log):
    ...
    for src in world.my_planets:
        ...
        if best is None:
            continue
        aim = aim_at_target(src, best, avail, world.initial_by_id, world.ang_vel)
        if aim is None:
            continue
        angle, turns = aim
...
def _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log):
    ...
    actions = search_step_action(
        ...
        max_actions_to_eval=18,  # V13.2y: was 12 — wider eval set
        use_depth2=SEARCH_DEPTH2_ENABLED,
        joint_refine=True,
        joint_refine_top=8,  # V13.2o: was 6 — even wider synergy discovery
    )
...
# ============================================================
# Mode 5 — V13.2a Reinforce-front terminal flush
# ============================================================

def _compute_frontier(world):
    """Frontier set = friendly planets that are the closest friend to some
    enemy planet, OR (if no enemies exist yet) the closest friend to some
    neutral. Returns set of planet ids.
    """
    if not world.my_planets:
        return set()
    ...
    if not targets:
        # Early game: use neutrals as the "front" so we still push toward
        # contested space rather than back to safe rear.
        targets = [p for p in world.planets if p.owner == -1]
    if not targets:
        return set()
    front = set()
    for t in targets:
        best = None
        best_d = float("inf")
        for fp in world.my_planets:
            d = dist(fp.x, fp.y, t.x, t.y)
            if d < best_d:
                best_d = d
                best = fp
        if best is not None:
...
def handle_reinforce_front(world, available, spent, target_locked, moves, mode_log):
    """V13.2a — Mandatory terminal stage that flushes leftover ships from
    rear planets to the nearest frontier-friendly planet. Cures the rear-
    stockpile paralysis pattern where idle rear planets pile ships that never
    enter the fight.

    Source = friendly planet NOT on the frontier, with leftover ships after
    all other modes, not in mode_log (or only absorb), not a promoted
    stockpile (saved for hammer), not contributing to a pending plan.

    Target = nearest frontier-friendly planet (raw distance).

    Send a fraction of the leftover (keeps half for next-turn flexibility
    so we don't strip planets that may need to defend or hammer-contribute).
    """
    if not REINFORCE_FRONT_ENABLED:
        return
    if world.step < REINFORCE_FRONT_MIN_STEP:
        return
    # Late-game blackout — don't ship reinforcements that can't repay.
    if LAUNCH_BLACKOUT_ENABLED and world.step >= TOTAL_STEPS - LAUNCH_BLACKOUT_TURNS:
        return
    if len(world.my_planets) < 2:
        return

    front = _compute_frontier(world)
    if not front:
        return
    front_planets = [p for p in world.my_planets if p.id in front]
    if not front_planets:
        return

    # Build a set of source ids participating in any pending hammer plan or
    # already-locked commitment — don't strip ships earmarked for offense.
    hammer_contributors = set()
    global _hammer_plan
    if _hammer_plan is not None:
        for contrib in _hammer_plan.get("contributors", []):
            sid = contrib.get("source_id")
            if sid is not None:
                hammer_contributors.add(int(sid))

    for src in world.my_planets:
        if src.id in front:
            continue
        # Already used by another mode this turn? Absorb-only is OK (just
        # means defense flagged it; doesn't preclude redistribution).
        status = mode_log.get(src.id)
        if status and "absorb" not in status:
            continue
        # Don't strip stockpiles being saved for a future hammer.
        if src.id in _promoted_stockpiles:
            continue
        if src.id in hammer_contributors:
            continue
        # Skip comets — they have their own evacuation logic.
        if src.id in world.comet_ids:
            continue

        leftover = max(0, available[src.id] - spent.get(src.id, 0))
        if leftover < REINFORCE_FRONT_MIN_SHIPS:
            continue

        frac = REINFORCE_FRONT_FRAC_2P if world.is_2p else REINFORCE_FRONT_FRAC
        to_send = max(REINFORCE_FRONT_MIN_SHIPS,
                      int(leftover * frac))
        if to_send > leftover:
            to_send = leftover
        if to_send < REINFORCE_FRONT_MIN_SHIPS:
            continue

        # V13.2dd: 2P-only smart targeting — send to friend closest to
        # ENEMY MASS, not just nearest friend. In 4P this overcommitted
        # to one flank and lost others, so gate.
        if world.is_2p:
            enemy_targets = [p for p in world.planets
                             if p.owner != world.player and p.owner != -1]
            if not enemy_targets:
                enemy_targets = [p for p in world.planets if p.owner == -1]
            best = None
            best_score = float("inf")
            for fp in front_planets:
                if fp.id == src.id:
                    continue
                d_from_src = dist(src.x, src.y, fp.x, fp.y)
                if enemy_targets:
                    d_to_enemy = min(dist(fp.x, fp.y, e.x, e.y) for e in enemy_targets)
                else:
                    d_to_enemy = 0.0
                score = d_from_src + 0.5 * d_to_enemy
                if score < best_score:
                    best_score = score
                    best = fp
        else:
            # 4P: nearest frontier friend (original behavior).
            best = None
            best_d = float("inf")
            for fp in front_planets:
                if fp.id == src.id:
                    continue
                d = dist(src.x, src.y, fp.x, fp.y)
                if d < best_d:
                    best_d = d
                    best = fp
        if best is None:
        ...
        aim = aim_at_target(src, best, to_send, world.initial_by_id, world.ang_vel)
        if aim is None:
            continue
        angle, turns = aim
        if turns > REINFORCE_FRONT_MAX_TRAVEL:
            continue

        _commit_fleet(world, moves, spent, target_locked,
                      src.id, best.id, angle, turns, int(to_send))
        mode_log[src.id] = "reinforce-front"
...
def plan_moves(world, deadline=None):
    ...
    # Mode 4c — V13.2a Reinforce-front terminal flush. Mandatory anti-idle
    # stage: any rear (non-frontier) friendly planet with leftover ships
    # forwards a fraction to the nearest frontier-friendly planet.
    if not _over_budget():
        handle_reinforce_front(world, available, spent, target_locked, moves, mode_log)
