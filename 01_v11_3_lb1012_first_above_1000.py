...
EARLY_TURN_LIMIT = 40
OPENING_TURN_LIMIT = 80            # V11 T1a REVERTED: extended opening regressed when stacked with other changes
LATE_REMAINING_TURNS = 70          # was 60 — enter late-game earlier
VERY_LATE_REMAINING_TURNS = 25
TOTAL_WAR_REMAINING_TURNS = 55     # unchanged from original — window for preservation-mode
HARD_FLUSH_REMAINING_TURNS = 10    # V10.1: last N turns — also require fleet arrival > 500 for in-flight scoring
PARK_SAFETY_MARGIN_TURNS = 2       # V10.1: parked fleets must arrive at turn >= 500 + margin when in hard flush
...
SAFE_OPENING_TURN_LIMIT = 10       # V11 T1a REVERTED
ROTATING_OPENING_MAX_TURNS = 13    # V11 T1a REVERTED
...
POST_CAPTURE_FRONTIER_MARGIN = 12   # V11 T1c: extra garrison floor for neutral captures within FRONTIER_DIST of enemies
FRONTIER_DIST_UNITS = 22            # matches existing 22-unit frontier threshold used elsewhere
CAN_HOLD_SAFETY_MARGIN = 2          # V11 T2a: tie-break buffer in worst-case enemy-full-attack check
THREAT_FIELD_RADIUS = 22.0          # V11 T2c: Euclidean units — targets within this of a strong enemy get value penalty
THREAT_FIELD_PENALTY_WEIGHT = 0.08  # V11 T2c: per-enemy-ship penalty at d=0 (scales linearly to 0 at d=radius)
KINGMAKER_DISCOUNT_4P = 0.60        # V11 T3b: value multiplier for attacking a non-leader in 4P (teccles N-1 approximation)
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.is_hard_flush = self.remaining_steps < HARD_FLUSH_REMAINING_TURNS
...
def build_policy_state(world, deadline=None):
    ...
    for planet in world.my_planets:
        ...
        proactive_keep = 0
        # V11 T1b REVERTED: skipping proactive defense during opening regressed winrate when
        # stacked with other V11 changes. Restoring V10.2 behavior — always compute proactive
        # reserves. Research's "disable during opening" claim may hold in isolation but broke
        # here; revisit with single-change ablation tournaments.
        ...
        # V10.1: removed `if world.is_total_war: halve keeps`. Halving defensive reserves during
        # total_war made planets cheaper for enemies to capture (see ep_75357000 collapse at t=466-474
        # where planets fell to counter-barrages). Preservation-mode parking (below) handles outbound
        # ships; reserves stay at their normal computed level.

        # V11 T2b REVERTED: evacuation escape was built to support Melis-lite hard-reject;
        # without the hard-reject (T2a reverted), it rarely fires and the extra branch adds
        # complexity for no gain. Restoring V10.2 clamp-to-ships behavior.
...
def build_modes(world):
    ...
    # V11 T3a: MP-Mix mode selector (Zuckerman, Felner & Kraus 2009 IJCAI). Fires only in
    # 4P after turn 100 (research3 §2: turns 0-100 are non-aggression by convention — early
    # attacks kingmake other players). Td/To are ship-count thresholds proportional to total.
    is_mp_paranoid = False
    is_mp_offensive = False
    mp_leader_id = None
    if world.is_four_player and world.step > 100:
        strengths = sorted(world.owner_strength.items(), key=lambda kv: -kv[1])
        if len(strengths) >= 2:
            total_ships = sum(s for _, s in strengths)
            if total_ships > 0:
                leading_edge = strengths[0][1] - strengths[1][1]
                my_rank = 1
                for owner, s in strengths:
                    if owner != world.player and s > world.my_total:
                        my_rank += 1
                Td = 0.20 * total_ships
                To = 0.15 * total_ships
                if my_rank == 1 and leading_edge >= Td:
                    is_mp_paranoid = True
                    attack_margin_mult += 0.15  # over-garrison, refuse trades when leading
                elif my_rank >= 2 and leading_edge >= To:
                    is_mp_offensive = True
                    mp_leader_id = strengths[0][0]
                    attack_margin_mult -= 0.05  # tighter margin when I must attack the leader
    ...
    return {
        ...
        "is_mp_paranoid": is_mp_paranoid,
        "is_mp_offensive": is_mp_offensive,
        "mp_leader_id": mp_leader_id,
    }
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    # V11 T2c REVERTED: threat-field penalty discouraged too many captures; `is_contested`
    # checks elsewhere already handle the most obvious cases. Revisit with per-target caching.

    # V11 T3b: teccles (N-1) kingmaker discount. In 4P, attacking a non-leader peer
    # mostly benefits the other opponents who split the relative gain. Discount value
    # so the bot prefers attacking the actual threat (the leader) over easy peer captures.
    if (
        world.is_four_player
        and target.owner not in (-1, world.player)
        and modes.get("mp_leader_id") is not None
        and target.owner != modes["mp_leader_id"]
    ):
        value *= KINGMAKER_DISCOUNT_4P
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if world.is_four_player and world._weakest_enemy and target.owner == world._weakest_enemy:
        margin = max(0, margin - 2)
    # V11 T1c REVERTED: +12 frontier garrison hurt overall winrate. Counter-barrages are
    # real but the extra per-capture cost slowed expansion more than it saved captures.
    # Revisit when per-planet defense budget accounts for inbound friendly reinforcements.
...
def plan_moves(world, deadline=None):
    ...
    # === V10.2 HARD FLUSH (final HARD_FLUSH_REMAINING_TURNS only) ===
    # Engine scoring (verified from orbit_wars.py lines 698-710): score = planet_ships +
    # fleet_ships per owner; in-flight fleets count. In the very last turns, launching at
    # enemies risks crashing fleets into defenders pre-500 (ships destroyed, no gain).
    # Parking to farthest friendly with arrival > 500 guarantees the ships still count.
    # Narrow gate: only fires in final HARD_FLUSH_REMAINING_TURNS turns.
    if world.is_hard_flush and len(world.my_planets) >= 2 and allow_optional_phase():
        min_travel = world.remaining_steps + PARK_SAFETY_MARGIN_TURNS
        for src in world.my_planets:
            if expired():
                return finalize_moves()
            ...
            for dst in world.my_planets:
                if dst.id == src.id:
                    continue
                d = planet_distance(src, dst)
                if d <= best_dist:
                    continue
                aim = world.plan_shot(src.id, dst.id, atk_left)
                if aim is None:
                    continue
                _, turns, _, _ = aim
                if turns > min_travel:
