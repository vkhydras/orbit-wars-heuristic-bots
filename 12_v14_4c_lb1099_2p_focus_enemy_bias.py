    ...
    for victim_id, (deficit, deadline, victim) in rescue_needs.items():
        ...
        if PREEMPTIVE_DOOM_EVAC_ENABLED and (not PREEMPTIVE_DOOM_EVAC_2P_ONLY or world.is_2p):
            ...
            if world.is_2p or not PREEMPTIVE_EVAC_USE_LARGEST_SINGLE_ENEMY_4P:
                ...
                # Per-owner total, take the max — biggest single attacker.
                by_owner = defaultdict(int)
...
def _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log):
    ...
    # 1. Find best evac destination: rank by HYBRID score
    # (V14.2 Phase 3.11 v2: garrison-only rank was marginal — try hybrid that
    # weights both stockpile-size and production, normalized).
    friendly_candidates = []
    for dst in world.my_planets:
        ...
        if turns > DOOM_EVAC_MAX_TRAVEL:
            continue
        # Hybrid: garrison + production*5 (1 prod ~= 5 turns of growth).
        # Higher score = more attractive. Negate for ascending sort.
